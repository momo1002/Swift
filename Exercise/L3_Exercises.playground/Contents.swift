...
emptyDict = ["Anchovies": true, "Coconut": false, "Cilantro":true, "Liver": true]
...
for (myKey, myValue) in presidentialPetsDict {
    print("\(myKey) : \(myValue)")
}
//: ### Exercise 11
...
presidentialDogs.removeValue(forKey: "Bill Clinton")
presidentialDogs["Bill Clinton"] = "Buddy"
for (myKey, myValue) in presidentialDogs {
    print("\(myKey) : \(myValue)")
}
//: ### Exercise 12
...
let result = presidentialPetsDict["Barack Obama"]
if result != nil {
    print("Barack Obama walks \(result!) every morning.")
} else {
    print("The name is not in the presidentialPetsDict dictionary.")
}
...
let arr = studioAlbums.keys.filter{ $0.contains("Led Zeppelin") }
print(arr.count)
